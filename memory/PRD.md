...
### Pass 15 — SHN write rollback ✅ + Android pair-UI residue scrubbed (current pass)
- **Audit row 10 closed**: new `Services/_Shared/ElleShnVersionStore.{h,cpp}` provides `AtomicWriteWithVersioning` — SHA1 → idempotent short-circuit on identical bytes → snapshot prior bytes to `.shn_versions/<stem>/<stem>.<ms>.shn` → prune to last N → atomic `.tmp` + rename. `POST /api/shn/save` response now carries `previous_existed`, `previous_bytes`, `previous_hash`, `new_hash`, `version_snapshot`; per-file history log line gained `prev_hash->new_hash`. `ListVersions(absDir, name)` ready to wire into a follow-up admin route.
- **Android pair UI scrubbed**: deleted dead `PairScreen.kt` (232 lines), `ElleRoutes.PAIR`, `ElleApi::pair` Retrofit method, `PairRequest` / `PairingPayload` data classes. Cleaned `ElleNavHost` parameter list (`isPaired` / `onPaired` / `prefill` were never consumed); `MainActivity` updated. `grep -rn 'PairScreen\|PairRequest\|PairingPayload\|ElleRoutes.PAIR\|auth/pair' Tools/Android/app/src/main/` returns zero hits.
- **Wiring**: `ElleCore.Shared.vcxproj` compiles `ElleShnVersionStore.cpp`. NUDE CODE preserved.
- **Verification**: Composer 17 + Intuition 39 + Probability 85 + Language 1 + Shared **40** (+6 ShnVersionStore) = **182/182 local Linux ctest PASS**. SHA1 verified against `SHA1("hello") = aaf4c61ddcc5e8a2dabede0f3b482cd9aea9434d`. `shellcheck -S warning` clean across all `Tools/*.sh`.

### Pass 14 — Config drift sweep ✅ + D34 IPC chain smoke ✅
- **Config schema drift sweep (#12)** closed: built a Python sweeper that compares every `ElleConfig::Instance().Get*("key", default)` call against the flattened `elle_master_config.json`. Added **30 missing keys** to their canonical sections (`bonding.*`, `cognitive.*` timeouts, `consent.*`, `dream.*`, `family.child_*`, `goals.fallback_*`, `http_server.game_db_*`, `imagination.*`, `innerlife.*`, `probability.*`, `services.named_pipes.max_payload_bytes`, `video.avatar_dir`). **Zero true mismatches remain** — all 17 raw mismatches classified as false positives (windows-path-escape-equivalent, unparseable-constexpr, unresolved-local-variable, intentional-testing-mode `no_auth`). Full report at `Docs/CONFIG_SCHEMA_DRIFT_REPORT.md`.
- **D34 cross-service IPC chain smoke** closed: new `Tools/ipc_chain_smoke.sh` POSTs `/api/ai/chat`, asserts HTTP 200 within 60 s, and verifies the response payload carries non-trivial output from every downstream service (`.response`, `.probabilistic_read`, `.inner_voice`, `.gut_read`, `.memories_used`, `.latency_ms`, `.provider_used`, `.model_used`). Optional heartbeat freshness check against `ElleSystem.dbo.Workers` for 7 expected service names when `ELLE_SQL_DSN`+`sqlcmd` are available.
- **CI hardening**: `.github/workflows/ctest-smoke.yml` got a new `tools-shellcheck` job running `bash -n` + `shellcheck -S warning` against all three smoke scripts on every push. All three pass cleanly.
- **Verification**: 176/176 local Linux ctest still green; shellcheck clean across all three `Tools/*.sh`. Audit rows 12 and D34 both → ✅ CLOSED.

### Pass 13 — SQL Fallback Phase 3: reaper auto-load + observability
...
- Tests: 176/176 local Linux ctest still green (no test count change — Phase 2 is C++ + SQL only, the new admin routes are exercised via the curl smoke matrix on Windows). `ElleSQLFallback.cpp` clean standalone-compile under `g++ -Wall -Wextra`.

### Pass 11 — HTTP fully split + SQL fallback durability hardening
- **HTTP Phase A** landed: `ElleHTTPService` class declaration extracted into `Services/Elle.Service.HTTP/HTTPServer.h`. File-private types (`HTTPRequest`, `HTTPResponse`, `RouteDispatch`, `WSClient`, `PendingChat`, `ChatCorrelator`, …) and file-scope free helpers moved alongside. Stateful globals promoted to `inline` for ODR safety across the new TUs. `HTTPServer.cpp` now `#include "HTTPServer.h"` with out-of-class definitions for the 27 non-route methods.
- **HTTP Phase C** landed: Each of the 18 `RegisterXxxRoutes()` bodies moved into its own translation unit (`HTTPServer_IntroRoutes.cpp` … `HTTPServer_SHNRoutes.cpp`). 158 routes preserved verbatim; brace total 1 902/1 902 conserved across the 19 .cpp files. `Elle.Service.HTTP.vcxproj` updated to compile all 19 .cpp files and list `<ClInclude>HTTPServer.h</ClInclude>`. **Needs Windows MSVC verification.**
- **SQL fallback durability — Phase 1** landed: new header-only `Services/_Shared/ElleSQLFallbackClassifier.h` with `ClassifyExec` and `ClassifyCallProc` heuristics. `ElleSQLFallback` JSONL line format now carries `idem` + `retry_count`. `DrainNow()` quarantines non-idempotent failures to `<exe>/sqllogs/poison/YYYY-MM-DD.txt` immediately; idempotent/unknown failures are retried up to `m_maxRetries` (default 5) before quarantine. Legacy `Enqueue(Kind, sql, params)` signature preserved (auto-classifies). New API: `EnqueueWithHint`, `PoisonBytes`, `PoisonFileCount`, `SetMaxRetries`.
- Tests: Shared ctest **26 → 34 PASS** (+8 classifier cases). Composer 17, Intuition 39, Probability 85, Language 1, Shared 34 — **176/176 total local Linux ctest green**. `ElleSQLFallback.cpp` standalone-compiled under `g++ -Wall -Wextra` with stub deps — clean.

### Pass 10 — HTTP Phase B + Multiplex belief mirror
