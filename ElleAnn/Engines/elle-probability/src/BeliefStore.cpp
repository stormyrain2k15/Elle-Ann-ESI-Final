// ============================================================================
// Elle Probability Engine -- Belief Store implementation
// File: src/BeliefStore.cpp
// ============================================================================
                                  ...
                                  double             halfLifeSecs)
{
    ...
    if (m_beliefs.count(domain)) return;   // already exists
...
{
    auto it = m_beliefs.find(domain);
    if (it == m_beliefs.end()) return;   // unknown domain; caller must register first

    // Apply decay before updating so the new evidence is applied to the
    // decayed posterior, not the stale one.
...
void BeliefStore::workerLoop() {
    while (true) {
        ...
        // Decrement in-flight and notify flush() waiters.
...
// ---------------------------------------------------------------------------
// Decay loop (runs every 30 seconds)
// ---------------------------------------------------------------------------
