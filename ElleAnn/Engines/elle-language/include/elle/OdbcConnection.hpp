// ============================================================================
// Elle Engine -- ODBC connection wrapper
// File: include/elle/OdbcConnection.hpp
//
// RAII handles for SQLHENV / SQLHDBC / SQLHSTMT. Designed for Microsoft ODBC
// Driver 18 for SQL Server. Connects via an explicit connection string built
// from EngineConfig::DatabaseConfig (Server, Database, Trusted_Connection,
// Encrypt, TrustServerCertificate, etc.).
//
// All input parameters use SQLBindParameter -- no string concatenation. Text
// values cross the boundary as SQL_C_WCHAR (UTF-16). Engine-side strings are
// UTF-8.
// ============================================================================
#pragma once
...
// Throws OdbcException on rc != SQL_SUCCESS && != SQL_SUCCESS_WITH_INFO.
void checkRc(SQLRETURN rc, SQLSMALLINT handleType, SQLHANDLE handle, const char* op);

// RAII wrapper around SQLHENV / SQLHDBC / SQLHSTMT.
...
public:
    ...
    // Parameter binding helpers (typed). Each appends to the parameter list.
    class Statement {
    public:
        ...
        // Bind helpers (all use SQLBindParameter). Order matches '?' placeholders.
        ...
        // Execute and step rows. Returns false when no more rows.
        void execute();
        bool fetch();

        // Column readers (1-based). Throws on out-of-range or convert errors.
        ...
        [[nodiscard]] std::string  getString(SQLUSMALLINT col);          // NVARCHAR -> UTF-8
    ...
    private:
        ...
        // Holders to keep parameter buffers alive until execute() returns.
