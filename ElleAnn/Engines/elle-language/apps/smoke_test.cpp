// ============================================================================
// Elle Engine -- Smoke test
// File: apps/smoke_test.cpp
//
// Quick canary that exercises every layer without any test framework.
// Returns 0 on success, non-zero on first failure. CI runs this before
// invoking the doctest unit-test suite.
// ============================================================================
...
int main() {
    ...
    // 2. Homonym distinction by context hint.
    {
        elle::ConversationContext baseball;
        baseball.activeContextHints.push_back(elle::ContextID{5});  // BASEBALL_CONTEXT
        const auto r = engine.analyze("The bat is heavy.", baseball);
        bool sawBat = false;
        for (const auto& rs : r.meaning.resolvedSenses) {
            if (rs.chosenSenseId && rs.chosenSenseId->value() == 5 /*bat#5 baseball club*/) sawBat = true;
        }
    ...
    }
    {
        ...
        wildlife.activeContextHints.push_back(elle::ContextID{6});  // WILDLIFE_CONTEXT
        const auto r = engine.analyze("The bat flew at dusk.", wildlife);
        bool sawBat = false;
        for (const auto& rs : r.meaning.resolvedSenses) {
            if (rs.chosenSenseId && rs.chosenSenseId->value() == 4 /*bat#4 mammal*/) sawBat = true;
        }
        REQUIRE(sawBat, "with WILDLIFE_CONTEXT hint, 'bat' must resolve to mammal sense (SenseID=4)");
    }

    // 3. Homophone independence.
    {
    ...
    // 6. Persistence is no-op for in-memory backend (returns 0, must not throw).
    {
        const auto r = engine.analyze("I'm fine.");
